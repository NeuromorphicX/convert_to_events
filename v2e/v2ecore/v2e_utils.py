...
import cv2
import glob
# --- headless-safe easygui import ---
# try:
#     # Default to headless on HPC; set V2E_HEADLESS=0 to allow GUI locally
#     if os.environ.get("V2E_HEADLESS", "1") == "1":
#         raise ImportError("headless")
#     import easygui as _easygui  # may pull in Tkinter on desktop
# except Exception:
#     _easygui = None

# class _EasyGUIProxy:
#     def __getattr__(self, name):
#         # Any accidental GUI call will error with a helpful message
#         def _no_gui(*a, **k):
#             raise RuntimeError(
#                 "easygui/Tk not available. Run v2e with CLI args "
#                 "(--input_video, --output_folder) instead of dialogs."
#             )
#         return _no_gui

# easygui = _easygui if _easygui is not None else _EasyGUIProxy()
# # --- end headless-safe block ---
# # ---------------- Headless-safe GUI shims ----------------
# import os

# Default to headless on servers. Set V2E_HEADLESS=0 to allow GUI locally.
_V2E_HEADLESS = os.environ.get("V2E_HEADLESS", "1") == "1"

# easygui shim (only used for file dialogs; optional)
try:
    if _V2E_HEADLESS:
        raise ImportError("headless")
...
class _EasyGUIProxy:
    def __getattr__(self, name):
        def _no_gui(*a, **k):
            raise RuntimeError(
                "GUI dialogs disabled (no Tk). Run v2e with CLI args "
                "(--input_video, --output_folder, etc.). "
                "Set V2E_HEADLESS=0 to enable GUI on a desktop with Tk."
            )
        return _no_gui

easygui = _easygui_mod if _easygui_mod is not None else _EasyGUIProxy()

# tkinter filedialog shim, lazy-loaded only if GUI explicitly enabled
def _load_filedialog():
    if _V2E_HEADLESS:
        raise RuntimeError(
            "Tkinter not available in headless mode. "
            "Use CLI args (no dialogs) or set V2E_HEADLESS=0 on a desktop with Tk."
        )
